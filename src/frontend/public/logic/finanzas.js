...
class FinanzasManager {
        ...
        return `
            <div class="space-y-4">
                <div class="flex items-center justify-between gap-4 pb-2 border-b border-[rgba(255,255,255,0.08)]">
                    <div>
                        <h1 class="text-2xl font-bold text-white">Finanzas</h1>
                        <p class="text-sm text-slate-400 mt-0.5">Control de ingresos y gastos</p>
                    </div>
                    <button id="btn-nueva-transaccion" class="btn-primary px-4 py-2 rounded-lg text-sm flex items-center gap-2">
                        <i class="fas fa-plus"></i> Nueva Transacción
                    </button>
                </div>
                <div class="grid grid-cols-3 gap-3">
                    <div class="glass rounded-lg p-4 card-lift">
                        <p class="text-xs text-slate-400 font-medium uppercase">Ingresos</p>
                        <p class="text-2xl font-bold text-emerald-400 mt-1" id="stat-ingresos">—</p>
                    </div>
                    <div class="glass rounded-lg p-4 card-lift">
                        <p class="text-xs text-slate-400 font-medium uppercase">Gastos</p>
                        <p class="text-2xl font-bold text-red-400 mt-1" id="stat-gastos">—</p>
                    </div>
                    <div class="glass rounded-lg p-4 card-lift">
                ...
                <div id="form-fin-wrapper" class="hidden glass rounded-lg p-5 border border-[rgba(43,147,166,0.3)]">
                    <h2 class="text-base font-bold text-white mb-3" id="form-fin-titulo">Nueva Transacción</h2>
                    <form id="form-finanzas" class="grid grid-cols-2 gap-3">
                        ...
                        <div>
                            ...
                            <input type="text" id="f-concepto" required placeholder="Descripción" class="w-full px-3 py-2 bg-[rgba(255,255,255,0.05)] border border-[rgba(255,255,255,0.1)] text-white placeholder-slate-500 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#2B93A6]">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-slate-300 mb-1">Tipo *</label>
                            <select id="f-tipo" required class="w-full px-3 py-2 bg-[rgba(255,255,255,0.05)] border border-[rgba(255,255,255,0.1)] text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#2B93A6]">
                                <option value="" class="bg-[#0f1e35]">Selecciona...</option>
                                <option value="ingreso" class="bg-[#0f1e35]">Ingreso</option>
                                <option value="gasto" class="bg-[#0f1e35]">Gasto</option>
                            </select>
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-slate-300 mb-1">Importe (€) *</label>
                            <input type="number" id="f-importe" required placeholder="0.00" min="0" step="0.01" class="w-full px-3 py-2 bg-[rgba(255,255,255,0.05)] border border-[rgba(255,255,255,0.1)] text-white placeholder-slate-500 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#2B93A6]">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-slate-300 mb-1">Fecha</label>
                            <input type="date" id="f-fecha" class="w-full px-3 py-2 bg-[rgba(255,255,255,0.05)] border border-[rgba(255,255,255,0.1)] text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#2B93A6]">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-slate-300 mb-1">Categoría</label>
                            <input type="text" id="f-categoria" placeholder="Ej: Servicios, Tintes..." list="f-categoria-opciones"
                            ...
                            <datalist id="f-categoria-opciones">
                                <option value="Servicios">
                                <option value="Tintes y coloración">
                                <option value="Cortes">
                                <option value="Tratamientos capilares">
                                <option value="Peinados">
                                <option value="Material y productos">
                        ...
                        <div class="flex items-end">
                            <div class="flex gap-2 w-full">
                                ...
                                <button type="button" id="btn-cancelar-fin" class="btn-secondary flex-1 px-4 py-2 rounded-lg text-sm">Cancelar</button>
                            </div>
                        </div>
                    </form>
                </div>
    ...
    _renderStats() {
        ...
        if (el('stat-balance')) {
            ...
            el('stat-balance').className = 'text-2xl font-bold mt-1 ' + (balance >= 0 ? 'text-emerald-400' : 'text-red-400');
        }
    }
    ...
    _renderLista({ deleteDoc, doc, db, uid, load }) {
        ...
        if (!lista) return;
        if (!this.transacciones.length) { lista.innerHTML = '<p class="text-slate-500 text-sm p-6 text-center">No hay transacciones registradas.</p>'; return; }
        ...
        lista.innerHTML = this.transacciones.map(t => {
            ...
            <div class="flex items-center justify-between p-4 border-b border-[rgba(255,255,255,0.06)] last:border-0 hover:bg-[rgba(43,147,166,0.06)] transition">
                <div class="min-w-0">
                    <p class="font-semibold text-white text-sm">${this._esc(t.concepto||'—')}</p>
                    <div class="flex flex-wrap gap-2 mt-1">
                        <span class="text-xs px-2 py-0.5 rounded-full font-medium ${esIngreso?'bg-emerald-500/20 text-emerald-300':'bg-red-500/20 text-red-300'}">${t.tipo}</span>
                        ${t.categoria?`<span class="text-xs px-2 py-0.5 rounded-full bg-[rgba(255,255,255,0.06)] text-slate-400">${this._esc(t.categoria)}</span>`:''}
                    </div>
                    <p class="text-xs text-slate-500 mt-1">${t.fecha||''}</p>
                </div>
                <div class="flex items-center gap-3 ml-3">
                    <p class="text-base font-bold ${esIngreso?'text-emerald-400':'text-red-400'}">${esIngreso?'+':'-'}${fmt(t.importe)}</p>
                    <div class="flex gap-1">
                        <button data-edit="${t.id}" class="text-xs border border-[rgba(43,147,166,0.4)] px-2 py-1 rounded-md text-[#38BDF8] hover:bg-[rgba(43,147,166,0.15)] transition">Editar</button>
                        <button data-del="${t.id}" class="text-xs border border-red-500/30 px-2 py-1 rounded-md text-red-400 hover:bg-red-500/10 transition">Eliminar</button>
                    </div>
                </div>
            </div>
        `}).join('');
