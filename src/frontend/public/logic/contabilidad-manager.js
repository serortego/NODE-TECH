// contabilidad-manager.js — ContabilidadManager para el SPA de inicio.html
// Usa window.db, window.fs y window.firebaseUser expuestos por auth-guard.js
// NO es un ES module: se carga con <script src="...">
...
class ContabilidadManager {
    ...
    async setupListeners() {
        ...
        // Exponer métodos de acción como globales (para onclick en HTML dinámico)
        ...
        // Delegación en tabla de conceptos
    ...
    _updateTotals() {
        ...
        const baseEl    = document.getElementById('cont-t-base');
        const ivaLblEl  = document.getElementById('cont-t-iva-label');
        const ivaEl     = document.getElementById('cont-t-iva');
        const totalEl   = document.getElementById('cont-t-total');
        if (baseEl)   baseEl.textContent   = this._fmtEur(base);
        if (ivaLblEl) ivaLblEl.textContent = `IVA (${iva}%)`;
        if (ivaEl)    ivaEl.textContent    = this._fmtEur(ivaAmount);
        if (totalEl)  totalEl.textContent  = this._fmtEur(base + ivaAmount);
    }
    ...
    _generatePDF(factura) {
        ...
        if (factura.fechaVencimiento) {
            pdfdoc.text(`Vence: ${factura.fechaVencimiento}`, 195, 24, { align: 'right' });
        }
