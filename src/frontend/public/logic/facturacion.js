// facturacion.js — Módulo de facturas para NODE
// Clase: FacturacionManager
// Gestiona la creación, edición, emisión y exportación PDF de facturas.
...
class FacturacionManager {
    ...
    render() {
        return `
        <div id="fact-root" class="space-y-4">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div class="glass rounded-lg p-4 card-lift">
                    ...
                    <p class="text-2xl font-bold text-white mt-1" id="fact-stat-total">—</p>
                </div>
                <div class="glass rounded-lg p-4 card-lift">
                    <p class="text-xs text-slate-400 font-medium uppercase">Pendiente</p>
                    <p class="text-2xl font-bold text-[#38BDF8] mt-1" id="fact-stat-pendiente">—</p>
                </div>
                <div class="glass rounded-lg p-4 card-lift">
                    <p class="text-xs text-slate-400 font-medium uppercase">Cobrado</p>
                    <p class="text-2xl font-bold text-emerald-400 mt-1" id="fact-stat-cobrado">—</p>
                </div>
                <div class="glass rounded-lg p-4 card-lift">
                    <p class="text-xs text-slate-400 font-medium uppercase">Vencidas</p>
                    <p class="text-2xl font-bold text-red-400 mt-1" id="fact-stat-vencidas">—</p>
                </div>
            </div>

            <!-- Formulario nueva/editar factura (oculto por defecto) -->
            <div id="fact-form-wrapper" class="hidden glass rounded-lg p-5 border border-[rgba(43,147,166,0.3)] space-y-4">
                <div class="flex items-center justify-between">
                    <h2 class="text-base font-bold text-white" id="fact-form-titulo">Nueva factura</h2>
                    <button id="fact-btn-cancelar"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                        ...
                        <input type="text" id="fact-f-cliente-nombre" placeholder="Nombre o empresa"
                    ...
                    <div>
                        ...
                        <input type="email" id="fact-f-cliente-email" placeholder="cliente@empresa.com"
                            class="w-full px-3 py-2 bg-[rgba(255,255,255,0.05)] border border-[rgba(255,255,255,0.1)] text-white placeholder-slate-500 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#2B93A6]">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-300 mb-1">NIF / CIF</label>
                        <input type="text" id="fact-f-cliente-nif" placeholder="B12345678"
                            class="w-full px-3 py-2 bg-[rgba(255,255,255,0.05)] border border-[rgba(255,255,255,0.1)] text-white placeholder-slate-500 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#2B93A6]">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-300 mb-1">Dirección</label>
                        <input type="text" id="fact-f-cliente-dir" placeholder="Calle, número, ciudad"
                            class="w-full px-3 py-2 bg-[rgba(255,255,255,0.05)] border border-[rgba(255,255,255,0.1)] text-white placeholder-slate-500 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#2B93A6]">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div>
                        ...
                        <input type="date" id="fact-f-fecha-emision"
                ...
                <div>
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <button id="fact-btn-add-concepto" type="button"
                    ...
                    <div class="overflow-x-auto rounded-lg border border-[rgba(255,255,255,0.08)]">
                        <table class="w-full text-sm">
                            ...
                            <tbody id="fact-conceptos-body"></tbody>
                        </table>
                    </div>
                </div>
                ...
                <div class="flex flex-col items-end gap-2">
                    <div class="flex items-center gap-3">
                        ...
                        <input type="number" id="fact-f-iva" min="0" max="100" value="21"
                    ...
                    <div class="bg-[rgba(255,255,255,0.04)] border border-[rgba(255,255,255,0.08)] rounded-lg p-3 w-64 space-y-1 text-sm">
                        <div class="flex justify-between text-slate-400">
                            <span>Base imponible</span><span id="fact-t-base">0,00 €</span>
                        </div>
                        <div class="flex justify-between text-slate-400">
                            <span id="fact-t-iva-label">IVA (21%)</span><span id="fact-t-iva">0,00 €</span>
                        </div>
                        <div class="flex justify-between font-bold text-white border-t border-[rgba(255,255,255,0.08)] pt-1">
                            <span>Total</span><span id="fact-t-total">0,00 €</span>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea id="fact-f-notas" rows="2"
                ...
                <div class="flex gap-2 flex-wrap">
                    ...
                    <button id="fact-btn-pdf" disabled
    ...
    async setupListeners() {
        ...
        if (!db || !uid) {
            const el = document.getElementById('fact-facturas-lista');
        ...
        this._saveFactura = async (estadoForzado) => {
            const nombre = document.getElementById('fact-f-cliente-nombre')?.value.trim();
            ...
            const iva   = parseFloat(document.getElementById('fact-f-iva')?.value) || 0;
            const base  = this.conceptos.reduce((s, c) => s + c.subtotal, 0);
            const total = base + base * (iva / 100);

            const data = {
                estado:           estadoForzado || document.getElementById('fact-f-estado')?.value || 'borrador',
                cliente: {
                    nombre,
                    email:     document.getElementById('fact-f-cliente-email')?.value.trim() || '',
                    nif:       document.getElementById('fact-f-cliente-nif')?.value.trim() || '',
                    direccion: document.getElementById('fact-f-cliente-dir')?.value.trim() || ''
                },
                fechaEmision:     document.getElementById('fact-f-fecha-emision')?.value || '',
                fechaVencimiento: document.getElementById('fact-f-fecha-vencimiento')?.value || '',
                conceptos:        this.conceptos.map(c => ({ ...c })),
                subtotal:         base,
                impuesto:         iva,
                total,
                notas:            document.getElementById('fact-f-notas')?.value.trim() || '',
                userId:           uid
            };
        ...
        // Botón nueva factura
        document.getElementById('fact-btn-nueva')?.addEventListener('click', () => this._showForm(null));
        document.getElementById('fact-btn-cancelar')?.addEventListener('click', () => this._hideForm());
        document.getElementById('fact-btn-add-concepto')?.addEventListener('click', () => this._addConcepto());
        document.getElementById('fact-filtro-estado')?.addEventListener('change', () => this._renderLista());
        document.getElementById('fact-f-iva')?.addEventListener('input', () => this._updateTotals());

        document.getElementById('fact-btn-guardar')?.addEventListener('click', () => {
            this._saveFactura(document.getElementById('fact-f-estado')?.value);
        });
        document.getElementById('fact-btn-guardar-emitir')?.addEventListener('click', () => {
            if (document.getElementById('fact-f-estado')) document.getElementById('fact-f-estado').value = 'emitida';
            this._saveFactura('emitida');
        });
        document.getElementById('fact-btn-pdf')?.addEventListener('click', () => {
            if (!this.editingId) return;
            const f = this.facturas.find(x => x.id === this.editingId);
            if (f) this._generatePDF(f);
        });

        document.getElementById('fact-conceptos-body')?.addEventListener('input', e => {
            const el = e.target;
            const i  = parseInt(el.dataset.i);
            if (!isNaN(i)) this._updateConcepto(i, el.dataset.f, el.value);
        });
        document.getElementById('fact-conceptos-body')?.addEventListener('click', e => {
    ...
    _showForm(factura) {
        ...
        document.getElementById('fact-form-titulo').textContent = factura ? `Editar ${factura.numero || 'factura'}` : 'Nueva factura';
        document.getElementById('fact-f-cliente-nombre').value    = factura?.cliente?.nombre    || '';
        document.getElementById('fact-f-cliente-email').value     = factura?.cliente?.email     || '';
        document.getElementById('fact-f-cliente-nif').value       = factura?.cliente?.nif       || '';
        document.getElementById('fact-f-cliente-dir').value       = factura?.cliente?.direccion || '';
        document.getElementById('fact-f-fecha-emision').value     = factura?.fechaEmision       || today;
        document.getElementById('fact-f-fecha-vencimiento').value = factura?.fechaVencimiento   || in30;
        document.getElementById('fact-f-estado').value            = factura?.estado             || 'borrador';
        document.getElementById('fact-f-iva').value               = factura?.impuesto           ?? 21;
        document.getElementById('fact-f-notas').value             = factura?.notas              || '';
        const pdfBtn = document.getElementById('fact-btn-pdf');
        ...
        document.getElementById('fact-form-wrapper').classList.remove('hidden');
        document.getElementById('fact-form-wrapper').scrollIntoView({ behavior: 'smooth', block: 'start' });
    }

    _hideForm() {
        document.getElementById('fact-form-wrapper')?.classList.add('hidden');
        this.editingId = null;
        this.conceptos = [];
    }
    ...
    _renderConceptos() {
        const tbody = document.getElementById('fact-conceptos-body');
        if (!tbody) return;
        tbody.innerHTML = '';
    ...
    _updateTotals() {
        ...
        set('fact-t-base',      this._fmtEur(base));
        set('fact-t-iva-label', `IVA (${iva}%)`);
        set('fact-t-iva',       this._fmtEur(ivaAmount));
        set('fact-t-total',     this._fmtEur(base + ivaAmount));
    }

    _renderLista() {
        const container = document.getElementById('fact-facturas-lista');
        if (!container) return;
        const filtro   = document.getElementById('fact-filtro-estado')?.value || '';
          ...
          <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-4 py-3 border-b border-[rgba(255,255,255,0.06)] hover:bg-[rgba(43,147,166,0.06)] transition last:border-0">
            ...
            <div class="flex items-center gap-3 flex-shrink-0">
              ...
              <div class="flex gap-1">
                <button onclick="window._factMgr._showForm(window._factMgr.facturas.find(x=>x.id==='${f.id}'))"
                ...
                <button onclick="window._factMgr._generatePDF(window._factMgr.facturas.find(x=>x.id==='${f.id}'))"
                ...
                <button onclick="window._factMgr._deleteFactura('${f.id}')"
                  class="border border-red-500/30 px-2.5 py-1 rounded-lg text-xs font-semibold text-red-400 hover:bg-red-500/10 transition">
                  <i class="fas fa-trash-alt"></i>
                </button>
              </div>
    ...
    _renderStats() {
        ...
        const set = (id, val) => { const el = document.getElementById(id); if (el) el.textContent = val; };
        set('fact-stat-total',     this.facturas.length);
        set('fact-stat-pendiente', this._fmtEur(emitidas.reduce((s, f) => s + (f.total || 0), 0)));
        set('fact-stat-cobrado',   this._fmtEur(pagadas.reduce((s, f) => s + (f.total || 0), 0)));
        set('fact-stat-vencidas',  vencidas.length);
    }
...
// ─── Auto-init para página standalone facturacion.html ───────────────────────
document.addEventListener('appReady', async () => {
    if (window.navManager) return;
    const container = document.getElementById('fact-standalone');
    if (!container) return;
    const mgr = new FacturacionManager(null);
